...
from gpen_model_script_half import FullGenerator
...
class FaceGAN(object):
    ...
    def load_model(self, channel_multiplier=2, narrow=1):
        ...
        ###转TensorScript
        img_tensor = torch.randn(1, 3, self.resolution, self.resolution, device='cuda').half()
        traced_script_module_encoder = torch.jit.trace(self.model, (img_tensor))
        traced_script_module_encoder.save('gpen_half.pt')
