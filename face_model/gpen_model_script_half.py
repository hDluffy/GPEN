...
class EqualLinear(nn.Module):
    ...
    def forward(self, input):
        if self.activation:
            out = F.linear(input.float(), self.weight.float() * self.scale)
            out = fused_leaky_relu(out, self.bias.float() * self.lr_mul, device=self.device).half()
